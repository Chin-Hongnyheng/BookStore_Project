...
export class WishlistService {
  ...
  async update(userId: number, updateWishlistDto: UpdateWishlistDto): Promise<Wishlist[]> {
  ...
  async remove(userId: number, productId: number) {
    const result = await this.wishlistRepository.delete({ userId, product: { id: productId } });
