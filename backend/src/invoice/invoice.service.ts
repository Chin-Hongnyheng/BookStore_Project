...
export class InvoiceService {
  ...
  private getInvoiceNumber(orderId: number): string {
    return `INV-${String(orderId).padStart(6, '0')}`;
  }

  /**
   * Generate full detailed PDF invoice
   */
  async generatePDF(order: InvoiceOrderData): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < order.items.length; i++) {
        ...
        doc
          ...
          .text(
            this.sanitizeForPdf(item.product?.title || 'Unknown'),
            90,
            y + 5,
            { width: 220 },
          )
  ...
  /**
   * Generate PNG preview image (summary only — optimized for Telegram)
   */

  async generatePreviewImage(order: InvoiceOrderData): Promise<string> {
    const invoiceNumber = this.getInvoiceNumber(order.id);
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  /**
   * Strip characters not supported by Helvetica (keeps Basic Latin + Latin-1 Supplement).
   * Non-Latin scripts (Khmer, CJK, Arabic, etc.) are removed to avoid garbled output.
   */
  private sanitizeForPdf(str: string): string {
    if (!str) return '';
    ...
    return str
      ...
      .replace(/&[^ ]+/g, '') // remove broken & sequences
      .trim(); // ✅ DO NOT remove non-ASCII
  }
